                        ...
                        <div class="col-12 col-md-12 col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    @foreach($detailklasifikasi as $row)
                                    <form action="{{URL::to('/')}}/detailklasifikasi/{{$row->id}}" method="POST" class="needs-validation" novalidate="" enctype="multipart/form-data">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="id_kriteria" class="form-control" disabled>
                                            @foreach($kriteria as $key => $row2)
                                                <option value="{{$row2->id}}" {{$row->id_kriteria == $row2->id ? 'selected' : ''}}>{{$row2->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>Nilai</label>
                                        <input type="number" name="nilai" class="form-control" min=0 value="{{$row->nilai}}">
                                    </div>
